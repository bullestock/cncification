...
teeth = 85;			// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
profile = 12;		// 1=MXL 2=40DP 3=XL 4=H 5=T2.5 6=T5 7=T10 
						// 8=AT5 9=HTD_3mm 10=HTD_5mm 11=HTD_8mm 
						// 12=GT2_2mm 13=GT2_3mm 14=GT2_5mm
...
retainer_ht = 1.5;	// height of flange over pulley, standard = 1.5
...
shoff = pulley_t_ht+retainer_ht*2+eps;  // Z offset of handle shell above pulley body
...
// here is the actual pulley/knob object:

rotate([0,180,0]) shell();  // flip 180 to print without overhanging top

// =================================================================
// Timing Belt Pulley Functions
...
module pulley( belt_type , pulley_OD , tooth_depth , tooth_width )
{
    ...
    // add in shaft flat
    flat_diam = 5.45+0.2; // diameter at flat
    flat_offset = flat_diam - (motor_shaft/2); // offset from center
    ms2=motor_shaft/2;
    shaft_width=3;
    shaft_high=7;
    total_h = pulley_t_ht + pulley_b_ht;  // total height
    intersection() {
        translate([0,ms2/2+flat_offset,total_h/2]) cube([motor_shaft,ms2,total_h],center=true);
        cylinder(r=ms2*1.1,h=total_h,$fn=30);
    }
	...
	union()
	{
		...
		translate([0,0,pulley_b_ht-1]) 
		rotate ([0,0,360/(teeth*4)]) 
		cylinder(r=pulley_OD/2,h=pulley_t_ht+2, $fn=teeth*4);
	        
		//teeth - cut out of shaft
                if(1)  // 0 for draft (no teeth)
                {
		    for(i=[1:teeth]) 
	    ...
	    if ( retainer > 0 ) {translate ([0,0, pulley_b_ht + pulley_t_ht ]) 
		                 ...
		                 //bottom belt retainer
                                 translate ([0,0, 0]) 
		                 rotate_extrude($fn=teeth*4)  
		                 polygon([[0,0],[pulley_OD/2 + retainer_ht,0],[pulley_OD/2 , retainer_ht+.5],[0 , retainer_ht],[0,0]]);
		                }
